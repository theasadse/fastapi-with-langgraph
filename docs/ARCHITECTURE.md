...
| `app/agents/nodes.py` | Holds the node functions that perform intake, safety checks, planning, tool routing, synthesis, critique, repair, and finalization. |
| `app/agents/tools.py` | Provides local tool functions used by graph nodes: built-in research, safe arithmetic, and workspace inspection. |
...
6. Conditional edges route based on safety, pending tools, and critique results.
7. FastAPI returns `AgentResponse` with the final answer, artifacts, trace, and route history.
...
| --- | --- | --- |
| `/` | `GET` | Redirects to `/docs` so the browser opens the interactive API documentation. |
| `/health` | `GET` | Confirms the service is running. |
| `/agent/graph` | `GET` | Returns static node, edge, and Mermaid graph metadata. |
| `/agent/run` | `POST` | Runs the complex LangGraph agent for a user query. |
